...
export class CodeActionController extends Disposable implements IEditorContribution {
	...
	private async showCodeActionsFromLightbulb(actions: CodeActionSet, at: IAnchor | IPosition): Promise<void> {
		// Onepilot fork: never auto-apply an "AI fix" from the lightbulb (would launch inline chat).
		// Fall through to the normal code-action list instead. See docs/strip.
		if (!ONEPILOT_HIDE_AI && actions.allAIFixes && actions.validActions.length === 1) {
