...
function toDefaultAccountConfig(defaultChatAgent: IDefaultChatAgent | undefined): IDefaultAccountConfig {
	// Onepilot fork: there is no built-in chat agent (defaultChatAgent removed). Return an inert config
	// so the account service constructs without crashing. With empty provider ids it simply stays
	// "unavailable" (no network, no sign-in) — a normal state that non-AI consumers (update, policy,
	// assignment, gallery) already handle. See docs/strip.
	if (!defaultChatAgent) {
		return {
			preferredExtensions: [],
