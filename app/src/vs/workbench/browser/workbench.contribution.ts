	...
	registry.registerConfiguration({
		...
		'properties': {
			...
			'workbench.editor.empty.hint': {
				'type': 'string',
				'enum': ['text', 'hidden'],
				// Onepilot cockpit: clean workspace — no empty-editor command hint watermark. User-overridable.
				'default': 'hidden',
			...
			'workbench.secondarySideBar.defaultVisibility': {
				...
				// Onepilot cockpit: blank-canvas shell — secondary side bar hidden by default. User-overridable.
				'default': 'hidden',
			...
			[LayoutSettings.ACTIVITY_BAR_LOCATION]: {
				...
				// Onepilot cockpit: blank-canvas shell — Activity Bar location 'top' renders the view-switcher
				// INSIDE the side bar (sidebarPart.getCompositeBarPosition → CompositeBarPosition.TOP), so the
				// navigator shows/hides *together with* the side bar: the left edge is empty when collapsed, and
				// toggling the side bar open brings its navigation strip with it. (The global Accounts/Manage gear
				// relocates to the title bar.) Not 'hidden' — that strips the view-switcher permanently. User-overridable.
				'default': 'top',
