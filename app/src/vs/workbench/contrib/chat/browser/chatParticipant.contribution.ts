...
// Onepilot fork: the AI chat UI is intentionally hidden. Not registering the chat view means no
// activity-bar entry, no open command, and no keybinding; the container stays registered but its
// `hideIfEmpty: true` keeps it invisible. Chat services remain registered (DI intact). docs/strip.
if (!ONEPILOT_HIDE_AI) {
