...
// Onepilot fork: no AI chat status-bar entry (the "Copilot" indicator). See docs/strip.
if (!ONEPILOT_HIDE_AI) {
...
// Onepilot fork: don't register the chat global actions (Open Chat / modes / toggle …) — this removes
// their command-palette entries AND keybindings (Cmd+Ctrl+I, Cmd+Alt+I, Cmd+Shift+I). See docs/strip.
if (!ONEPILOT_HIDE_AI) {
	registerChatActions();
}
