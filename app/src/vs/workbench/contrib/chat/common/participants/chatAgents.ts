...
export class ChatAgentService extends Disposable implements IChatAgentService {
	...
	registerAgentImplementation(id: string, agentImpl: IChatAgentImplementation): IDisposable {
		...
		// Onepilot fork: ONEPILOT_HIDE_AI is the master switch for the built-in AI chat. Keeping
		// `chatIsEnabled` (ChatContextKeys.enabled) false ensures the ~59 `when`-clauses gated on it
		// stay hidden, regardless of which agents register. See docs/strip.
		if (entry.data.isDefault && !ONEPILOT_HIDE_AI) {
		...
		return toDisposable(() => {
			...
			if (entry.data.isDefault) {
				this._hasDefaultAgent.set(!ONEPILOT_HIDE_AI && Iterable.some(this._agents.values(), agent => agent.data.isDefault && !!agent.impl));
			}
		});
	}
