...
// Onepilot fork: AI Voice Mode is hidden. Gating these command/keybinding registrations means the
// feature cannot be enabled even via settings.json (the `agents.voice.enabled` setting otherwise
// gates the menu/precondition). Voice services above stay registered (unreachable). See docs/strip.
if (!ONEPILOT_HIDE_AI) {
