...
// Onepilot fork: the built-in AI onboarding is disabled (no `defaultChatAgent`). Do NOT throw at
// module load — fall back to an empty config and gate `show()` to a no-op so the fallback is never
// actually used (the onboarding never displays). See docs/strip.
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		if (ONEPILOT_HIDE_AI) {
			return; // Onepilot fork: built-in AI onboarding is disabled. See docs/strip.
		}
		if (this.overlay) {
			return;
		}
