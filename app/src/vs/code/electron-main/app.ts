...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Agent Host
		// Onepilot fork: never spawn the AI agent-host process (it backs Copilot/AI agent sessions).
		// See docs/strip. The downstream sessions providers become unreachable dead code.
		if (!ONEPILOT_HIDE_AI && isAgentHostEnabled(this.configurationService)) {
	...
	private async openFirstWindow(accessor: ServicesAccessor, initialProtocolUrls: IInitialProtocolUrls | undefined): Promise<ICodeWindow[]> {
		...
		// Onepilot fork: the AI agent-sessions window is disabled (the `--agents` route is a no-op). docs/strip.
		if (!ONEPILOT_HIDE_AI && args['agents']) {
